...
# Contributions by Scott M. Ransom (SMR), Paul B. Demorest (PBD), Emmanuel
#    Fonseca (EF), and Gosia Curylo (GC). 
...
# Enable LaTeX and configure fonts to Times New Roman
#plt.rcParams['text.usetex'] = True  # Enable LaTeX
plt.rcParams['font.family'] = 'serif'  # Use serif fonts
plt.rcParams['font.serif'] = ['Times New Roman']  # Set Times New Roman for normal text
...
#Default on-pulse mask threshold
sigma_on = 5

# If F0_fact == 0:
# Ignore 0-frequency (sum) component in Fourier fit  (it will make time-domain signal mean-centered or zero-centered), it removes the DC offset
# else set F0_fact == 1.
F0_fact = 0
...
class DataPortrait(object):
    ...
    def __init__(self, datafile=None, joinfile=None, state='Intensity', quiet=False,
                 **load_data_kwargs):
        """
        Unpack all of the data and set initial attributes.
        datafile: psrchive archive
        state: 'Intensity' or 'Stokes'

        Defualt usage: datafile is a single archive (e.g. average portrait or arbitrary archive). 
        ...
        If datafile is a metafile of PSRCHIVE archives, "join" attributes are
            ...
            unique receiver and state='Intensity'. 
        state: either "Intensity" (default) or 'Stokes'. If state='Stokes' datafile has to be a single archive. 
        ...
        ##################################################################
        ### If datafile is a list of files. Only for state='Intensity'
        ##################################################################
        self.init_params = []
        ...
        if file_is_type(datafile, "ASCII"):
            ...
            for ifile in range(len(self.datafiles)):
                ...
                self.noise_stdsxs.extend(data.noise_stds[0, 0][data.ok_ichans[0]])
        ...
        #######################################################################################################
        # WORK WITH DATA PORTRAIT (state: "Intensity" or "Stokes")
        #######################################################################################################
        else:
            self.njoin = 0
            self.join_params = []
            ...
            ps = True if state == 'Intensity' else False
            self.data = load_data(datafile, state=state, dedisperse=True,
                                  dededisperse=False, tscrunch=True, pscrunch=ps,
            ...
            self.freqsxs = [self.freqs[0, self.ok_ichans[0]]]  # nsub*[np.shape(nchanxs,)] as every pol of each subint will have same freqxs
            ...
            if state == 'Stokes':
                self.port = (self.masks * self.subints)                           #np.shape(nsub, npol, nchan, nbin), NOTE: here we're masking 0s! 
                shapexs = self.nsub, self.npol, len(self.ok_ichans[0]), self.nbin #shape of portrait excluding zapped channels 
                self.portx        = np.zeros(shapexs)       #np.shape(nsub,npol,nchan,nbin)
                self.flux_profx   = np.zeros(shapexs[:-1])  #np.shape(nsub,npol,nchan)
                self.noise_stdsxs = np.zeros(shapexs[:-1])
                self.SNRsxs       = np.zeros(shapexs[:-1]) 
                for isub in range(self.nsub):
                    for ipol in range(self.npol):        
                        self.portx[isub,ipol]        = self.port[isub,ipol][self.ok_ichans[0]]
                        self.flux_profx[isub,ipol]   = self.flux_prof[isub,ipol][self.ok_ichans[0]]
                        self.noise_stdsxs[isub,ipol] = self.noise_stds[isub,ipol,self.ok_ichans[0]]
                        self.SNRsxs[isub,ipol]       = self.SNRs[isub,ipol,self.ok_ichans[0]]

    def polarimetry(self, phase_rot=0, quiet=True):
        """ 
        Calculate basic polarimetric properties. 
        NOTE: currently assuming one subintegration!

        Input:
        phase_rot: float.
                   Might be needed to help on-pulse mask, as psrchive struggles when pulse is wrapped on phase edges.
        
        Output:
        PAx_ph, PAx_ph_sigma         : phase- and frequency-resolved position angle (nchan, nbin)
        Lfra_ph, Vfra_ph             : phase- and frequency-resolved fractions of L and V (nchan,nbin)
        Lfra, Lfra_err               : phase-averaged fraction of L and its uncertinty (nchan) (Oswald23 *)
        Vfra, Vfra_err               : phase-averaged fraction of V and its uncertinty (nchan) (Oswald23 *)
        absVfra, absVfra_err         : phase-averaged fraction of |V| and its uncertinty (nchan) (Oswald23 *)
        Lfra_bar, Lfra_bar_err       : phase-averaged fraction of L and its uncertainty  (nchan) (Oswald23 bar)
        Vfra_bar, Vfra_bar_err       : phase-averaged fraction of V and its uncertainty  (nchan) (Oswald23 bar)
        absVfra_bar, absVfra_bar_err : phase-averaged fraction of |V| and its uncertainty (nchan) (Oswald23 bar)
        LIstar, VIstar, absVIstar    : phase- and frequency-averaged fractions of L, V and |V| (Oswald23 *)
        LIbar, VIbar, absVIbar       : phase- and frequency-averaged fractions of L, V and |V| (Oswald23 bar)
        sigma_LIstar, sigma_LIbar... : uncertainties of the above
        nchanx_keep                  : IDs of non-zapped channels with at least one on-pulse bin
        """

        ##TODO: stardardize array shapes (nchan vs nchanx vs nchanx_keep)

        #Return the on-pulse mask per frequency channel
        self.nchanx = self.ok_ichans[0]
        self.get_onpulse_mask_f(phase_rot=phase_rot, quiet=quiet)
        if not np.any(self.onpulse_mask_f):
            raise RuntimeError("On-pulse mask failed (all False). Try phase_rot or check data.")
        #if np.all(self.onpulse_mask_f == False):
        #    print("ERROR: mask failed.")

        data   = self.subints 
        chanx  = self.nchanx
        nchanx = len(chanx)
        nsub,npol,nchan,nbin = data.shape
        self.weightsxs = self.weights[0][self.ok_ichans[0]]
        
        #Get Stokes data, each of np.shape(nchan,nbin); assuming one subintegration (load_data has tscrunch=True, should be fine!)
        I = data[0,0]
        Q = data[0,1]
        U = data[0,2]
        V = data[0,3]
        ...
        #Use only non-zapped channels
        Ix    = I[chanx]
        Qx    = Q[chanx]
        Ux    = U[chanx]
        Vx    = V[chanx]
        absVx = np.abs(Vx)   #np.shape(nchanx, nbin)

        #Debias linear polarisation (Muller17)
        wind_size = get_window_size(Ix.T.shape)

        Lx = debias_linpol_2d(Ix.T, Qx.T, Ux.T, window_size=wind_size,quiet=quiet)
        Lx = Lx.T
        L[chanx] = Lx   #don't need to debias zapped channels
    
        #######################################################################
        #Polarisation fractions, phase and frequency resolved
        #######################################################################
        LdivI = np.zeros_like(Ix)
        _ = np.divide(Lx, Ix, out=LdivI, where=Ix != 0)
        VdivI = np.zeros_like(Vx)
        _ = np.divide(Vx, Ix, out=VdivI, where=Ix != 0)
        LdivI_nan = np.where(LdivI == 0, np.nan, LdivI)
        VdivI_nan = np.where(VdivI== 0, np.nan, VdivI)

        #######################################################################
        #Polarisation fractions, phase-averaged and phase-frequency averaged
        #######################################################################

        #Find onpulse mask estimated from Stokes I with psrchive from the frequency averaged profile
        self.get_onpulse_mask(phase_rot)       
        om = self.onpulse_mask

        #Also psrchive on-pulse mask estimated from Stokes I, but per frequency channel
        om2    = self.onpulse_mask_f  
        om2x   = om2[self.nchanx]
        N_on   = om2x.sum(axis=1)     #Number of on_pulse bins per channel

        #Clip the data to only those channels which have onpulse bins
        keep = om2x.any(axis=1)                       # (nchanx_on,)
        if not np.any(keep):
            raise ValueError("No channels have on-pulse bins after zapping.")
        self.nchanx_keep = np.asarray(self.nchanx)[keep] # map to original ids
        ...
        def on_mean(x,mask=om2x):
            return np.nanmean(np.where(mask, x, np.nan), axis=1)
        def off_std(x,mask=om2x):
            return np.nanstd(np.where(~mask, x, np.nan), axis=1)

        #Mean is taken across onpulse bins estimated from I
        Ix_onpulse = np.where(om2x, Ix, np.nan)
        Qx_onpulse = np.where(om2x, Qx, np.nan)
        Ux_onpulse = np.where(om2x, Ux, np.nan)
        Vx_onpulse = np.where(om2x, Vx, np.nan)
        Lx_onpulse = np.where(om2x, Lx, np.nan)

        Ix_onpulse_mean    = on_mean(Ix_onpulse)
        Qx_onpulse_mean    = on_mean(Qx_onpulse)
        Ux_onpulse_mean    = on_mean(Ux_onpulse)
        Vx_onpulse_mean    = on_mean(Vx_onpulse)     
        Lx_onpulse_mean    = on_mean(Lx_onpulse)

        # off-pulse noise
        sigma_Ix_bin = off_std(Ix)
        sigma_Qx_bin = off_std(Qx)
        sigma_Ux_bin = off_std(Ux)
        sigma_Vx_bin = off_std(Vx)
        sigma_Lx_bin = off_std(Lx)
        self.sigmaI = sigma_Ix_bin 

        # standard errors of the on-pulse means
        with np.errstate(invalid='ignore', divide='ignore'):
            denom = np.sqrt(N_on)
            sigma_Ix = sigma_Ix_bin / denom
            sigma_Qx = sigma_Qx_bin / denom
            sigma_Ux = sigma_Ux_bin / denom
            sigma_Lx = sigma_Lx_bin / denom
            sigma_Vx = sigma_Vx_bin / denom

        ################################
        # |V| correction (Oswald23)
        #################################
        th = sigma_Ix_bin[:, None] * np.sqrt(2/np.pi)
        absVx_corr = np.maximum(absVx - th, 0) 

        self.absVx_corr = absVx_corr

        # On-pulse mean of corrected |V|; off-pulse std - SEM
        absVx_corr_onpulse_mean = on_mean(absVx_corr)                                 # (nchanx,)
        sigma_absVx_corr = off_std(absVx_corr) / denom                                # (nchanx,)

        ########################################################################################
        # Fractions, asterix (Oswald23)
        # First calculate on-pulse mean of I, Q, U, V and then fractions of L, V, absV
        ########################################################################################
        ...
        #########################
        with np.errstate(invalid='ignore', divide='ignore'):
            #linear_fraction       = Lx_onpulse_mean / Ix_onpulse_mean
            linear_fraction        = np.sqrt(Qx_onpulse_mean**2 + Ux_onpulse_mean**2) / Ix_onpulse_mean
            circular_fraction      = Vx_onpulse_mean / Ix_onpulse_mean
            abs_circular_fraction  = np.abs(circular_fraction) #|sumV / sumI| not sum|V|/sum|I|

        # Error propagation for fractions (per channel)
        r = np.sqrt(Qx_onpulse_mean**2 + Ux_onpulse_mean**2)      
        with np.errstate(invalid='ignore', divide='ignore'):
            sigma_linear_frac = np.sqrt( ((Qx_onpulse_mean / (Ix_onpulse_mean * r))**2 ) * sigma_Qx**2 +
                                         ((Ux_onpulse_mean / (Ix_onpulse_mean * r))**2 ) * sigma_Ux**2 +
                                         ((r / (Ix_onpulse_mean**2))**2 ) * sigma_Ix**2)
        with np.errstate(invalid='ignore', divide='ignore'):
            sigma_circular_fraction = np.sqrt((sigma_Vx / Ix_onpulse_mean)**2 +
                                              (Vx_onpulse_mean * sigma_Ix / (Ix_onpulse_mean**2))**2)
            sigma_abs_circular_fraction = sigma_circular_fraction.copy()

        #########################
        ## Frequency-averaged (again, taking only those channels which have on-pulse bins)
        #########################
        profI    = np.average(Ix,axis=0, weights=self.weightsxs[keep]) 
        profQ    = np.average(Qx,axis=0, weights=self.weightsxs[keep])
        profU    = np.average(Ux,axis=0, weights=self.weightsxs[keep])
        profL    = np.average(Lx,axis=0, weights=self.weightsxs[keep])
        profV    = np.average(Vx,axis=0, weights=self.weightsxs[keep])
        profabsV = np.average(absVx,axis=0, weights=self.weightsxs[keep])

        #Mask with psrchive
        oi_mask = om
        self.oi_mask_pr = oi_mask
        if not oi_mask.any():
            #Mask from Andrew's sigma_clip, used in case psrchive fails
            sigma_profI, oi, _, _, _ = sigma_clip(profI,sigma_th=2,quiet=quiet)
            oi_mask_and = np.zeros(nbin, dtype=bool)
            strong_inds = [idx for idx in oi if profI[idx] > sigma_on * sigma_profI] 
            oi_mask_and[strong_inds] = True
            self.sigma_profI = sigma_profI
            self.oi_mask_and = oi_mask_and
            oi_mask = oi_mask_and
            if not quiet:
                print("PSRCHIVE mask on average profile failed. If profile is wrapped on edges, rotate with phase_rot and try again.")
                print("Othersiwe, I will try another way...")
        self.oi_mask_pr = oi_mask

        ## Now average in phase
        profImean = profI[oi_mask].mean()
        profQmean = profQ[oi_mask].mean() 
        profUmean = profU[oi_mask].mean() 
        profLmean = profL[oi_mask].mean()
        profVmean = profV[oi_mask].mean()

        LIstar  = np.sqrt(profQmean**2 + profUmean**2) / profImean
        VIstar  = profVmean/profImean
        absVIstar = np.abs(VIstar)

        #Errs
        N_on_prof = int(oi_mask.sum())
        def off_std_1d(x, onmask):
            # std over off-pulse bins
            return np.nanstd(x[~onmask], ddof=1)
        sigma_profI_bin = off_std_1d(profI, oi_mask)
        sigma_profQ_bin = off_std_1d(profQ, oi_mask)
        sigma_profU_bin = off_std_1d(profU, oi_mask)
        sigma_profV_bin = off_std_1d(profV, oi_mask)
        # SEM of the on-pulse mean
        sigma_profImean = sigma_profI_bin / np.sqrt(N_on_prof)
        sigma_profQmean = sigma_profQ_bin / np.sqrt(N_on_prof)
        sigma_profUmean = sigma_profU_bin / np.sqrt(N_on_prof)
        sigma_profVmean = sigma_profV_bin / np.sqrt(N_on_prof)

        r = np.sqrt(profQmean**2 + profUmean**2)
        sigma_LIstar = np.nan
        sigma_VIstar = np.nan
        with np.errstate(divide='ignore', invalid='ignore'):
            # LIstar
            if (profImean != 0) and (r != 0):
                sigma_LIstar = np.sqrt((profQmean / (profImean * r))**2 * sigma_profQmean**2 +
                                       (profUmean / (profImean * r))**2 * sigma_profUmean**2 +
                                       (r / (profImean**2))**2       * sigma_profImean**2)
            # VIstar
            if profImean != 0:
                sigma_VIstar = np.sqrt((sigma_profVmean / profImean)**2 +
                                       (profVmean * sigma_profImean / (profImean**2))**2)
        sigma_absVIstar = sigma_VIstar

        ###########################################################
        # Fractions barred (Oswald23)
        # First calculate I, L, V and then the mean over phase
        ###########################################################

        #########################
        #Frequency-resolved
        #########################
        # Sums over on-pulse bins (per channel) of I, L and V
        S_Ix          = np.nansum(np.where(om2x, Ix,0.0), axis=1)
        S_Lx          = np.nansum(np.where(om2x, Lx,0.0), axis=1)
        S_Vx          = np.nansum(np.where(om2x, Vx,0.0), axis=1)
        S_absVx_corr  = np.nansum(np.where(om2x, absVx_corr,0.0), axis=1)
        with np.errstate(invalid='ignore', divide='ignore'):
            linear_fraction_bar       = S_Lx / S_Ix
            circular_fraction_bar     = S_Vx / S_Ix
            abs_circular_fraction_bar = S_absVx_corr / S_Ix
        # std of sums from SEMs of means
        sigma_SI      = sigma_Ix * N_on
        sigma_SLx     = sigma_Lx * N_on         
        sigma_SVx     = sigma_Vx * N_on   
        sigma_SabsVx = sigma_absVx_corr * N_on
        with np.errstate(invalid='ignore', divide='ignore'):
            sigma_linear_fraction_bar = np.sqrt((sigma_SLx / S_Ix)**2 +((S_Lx * sigma_SI) / (S_Ix**2))**2)
            sigma_circular_fraction_bar = np.sqrt((sigma_SVx / S_Ix)**2 + ((S_Vx * sigma_SI) / (S_Ix**2))**2)
            sigma_abs_circular_fraction_bar = np.sqrt((sigma_SabsVx / S_Ix)**2 + ((S_absVx_corr * sigma_SI) / (S_Ix**2))**2)

        ##################################
        ###Frequency-averaged
        ##################################
        profIx       = np.average(S_Ix, axis=0, weights=self.weightsxs[keep])
        profLx       = np.average(S_Lx, axis=0, weights=self.weightsxs[keep])
        profVx       = np.average(S_Vx, axis=0, weights=self.weightsxs[keep])
        profabsVx    = np.average(S_absVx_corr, axis=0, weights=self.weightsxs[keep])

        LIbar    = profLx/profIx
        absVIbar = profabsVx/profIx
        VIbar    = profVx/profIx

        #Errs
        w = self.weightsxs[keep].astype(float)
        wsum = np.sum(w)
        def sigma_weighted_mean(sig, w):
            return np.sqrt(np.sum((w**2) * (sig**2)) / (np.sum(w)**2))
        sigma_profIx    = sigma_weighted_mean(sigma_SI,     w)
        sigma_profLx    = sigma_weighted_mean(sigma_SLx,    w)
        sigma_profVx    = sigma_weighted_mean(sigma_SVx,    w)
        sigma_profabsVx = sigma_weighted_mean(sigma_SabsVx, w)
        sigma_LIbar = np.nan
        sigma_VIbar = np.nan
        sigma_absVIbar = np.nan
        with np.errstate(divide='ignore', invalid='ignore'):
            if profIx != 0:
                sigma_LIbar = np.sqrt((sigma_profLx / profIx)**2 +
                                      (profLx * sigma_profIx / (profIx**2))**2)
                sigma_VIbar = np.sqrt((sigma_profVx / profIx)**2 +
                                      (profVx * sigma_profIx / (profIx**2))**2)
                sigma_absVIbar = np.sqrt((sigma_profabsVx / profIx)**2 +
                                         (profabsVx * sigma_profIx / (profIx**2))**2)

        ##################################################################################
        #Position angle (PA) in [rad]
        ##################################################################################

        # averaged in phase (to add)
        

        # phase resolved (select on pulse in post analysis!)
        PAx_ph = 0.5 * np.arctan2(Ux, Qx)
        # High-SNR piece (P0 > 10), unequal noise allowed
        sigma_PA_eff = np.sqrt(0.5*(sigma_Qx_bin**2 + sigma_Ux_bin**2))  # (nchan,)
        #P0    = Lx / sigma_Ix_bin[:, None]     #less accurate if sigma_Q/U is very different than sigma_I, but it's a good approximation  
        P0    = Lx / sigma_PA_eff[:, None]
        den   = Qx**2 + Ux**2
        var_U = (sigma_Ux_bin[:, None])**2
        var_Q = (sigma_Qx_bin[:, None])**2
        with np.errstate(divide='ignore', invalid='ignore'):
            sigma_hi = 0.5 * np.sqrt(Qx**2 * var_U + Ux**2 * var_Q) / den
        hi = (P0 > 10.0) & (Lx > 0)
        #Low-SNR piece (Naghizadeh-Khouei&Clarke93)
        P0_grid      = np.linspace(0.0, 10.0, 400)
        sigma_grid   = np.array([sigma_pa_from_P0(p) for p in P0_grid])
        sigma_interp = interp1d(P0_grid, sigma_grid, kind='linear', bounds_error=False, fill_value=(sigma_grid[0], 0.5/10.0))
        sigma_lo     = sigma_pa_bins(Lx,sigma_PA_eff,sigma_interp) 
        PAx_ph_sigma = np.where(hi, sigma_hi, sigma_lo)

        ##################################################################################
        #Save linear portrait: np.shape(1,3,nchan,nbin), where 3 = I, L, V
        ##################################################################################
        statesL       = [I, L, V]                                       #np.shape(nchan,nbin)
        portL         = np.zeros((nsub,3,nchan,nbin))
        portLx        = np.zeros((nsub,3,nchanx,nbin))
        LSNRs         = np.zeros((nsub,3,nchan))
        LSNRsxs       = np.zeros((nsub,3,nchanx))
        Lnoise_stds   = np.zeros((nsub,3,nchan))
        Lnoise_stdsxs = np.zeros((nsub,3,nchanx))

        for isub in range(nsub):
            for ipol in range(3):
                portL[isub,ipol]         = statesL[ipol]
                portLx[isub,ipol]        = statesL[ipol][chanx]
                Lnoise_stds[isub, ipol]  = get_noise(statesL[ipol], chans=True)
                Lnoise_stdsxs[isub,ipol] = Lnoise_stds[isub, ipol][chanx]
                power_based=False if ipol==0 else True
                for ichan in range(nchan):
                    #Psrchive's snr normalization is a bit different, so they will not be the same!
                    om_onechan = om2[ichan]
                    LSNRs[isub,ipol,ichan] = get_SNR_better(statesL[ipol][ichan], onpulse_mask=om_onechan, threshold=0.10, use_pp_noise=False, 
                                                            power_based=power_based, frac=4, plot=False) 
                LSNRsxs[isub,ipol] = LSNRs[isub][ipol][chanx]
        
        Lflux_prof  = np.average(portL,axis=2, weights=self.weights[0])
        Lflux_profx = portLx.mean(axis=2) #This is not needed, weights are 0 for zapped channels so flux_prof=flux_profx

        results = DataBunch(portL=portL,portLx=portLx,LSNRs=LSNRs,LSNRsxs=LSNRsxs,
                            Lnoise_stds=Lnoise_stds,Lnoise_stdsxs=Lnoise_stdsxs,Lflux_prof=Lflux_prof, Lflux_profx=Lflux_profx, 
                            PAx_ph=PAx_ph, PAx_ph_sigma=PAx_ph_sigma,
                            Lfra_ph=LdivI_nan, Vfra_ph=VdivI_nan,
                            Lfra=linear_fraction, Lfra_err=sigma_linear_frac, 
                            Vfra=circular_fraction, Vfra_err=sigma_circular_fraction,
                            absVfra=abs_circular_fraction, absVfra_err=sigma_abs_circular_fraction,
                            Lfra_bar=linear_fraction_bar, Lfra_bar_err=sigma_linear_fraction_bar,
                            Vfra_bar=circular_fraction_bar, Vfra_bar_err=sigma_circular_fraction_bar,
                            absVfra_bar=abs_circular_fraction_bar, absVfra_bar_err=sigma_abs_circular_fraction_bar,
                            LIstar=LIstar,VIstar=VIstar,absVIstar=absVIstar,
                            LIbar=LIbar, VIbar=VIbar, absVIbar=absVIbar, 
                            sigma_LIbar=sigma_LIbar, sigma_VIbar=sigma_VIbar, sigma_absVIbar=sigma_absVIbar, 
                            sigma_LIstar=sigma_LIstar, sigma_VIstar=sigma_VIstar, sigma_absVIstar=sigma_absVIstar)
        
        self.POL = results
        self.__dict__.update(**self.POL)
        
        self.portL  = self.portL * self.masks[:,:3,:,:]
        self.portLx = portLx

    def get_onpulse_mask_f(self, phase_rot, quiet):
        """  
        Get an onpulse mask for each channel with psrstat minimum algorithm. 
        """

        #First try psrchive:
        chan_list = list(map(int, self.nchanx))                # non-zapped channels to measure
        chanx = ",".join(map(str, chan_list))
        psrstat_out = subprocess.run(
            ["psrstat", "-j", f"r {phase_rot}", "-l", f"chan={chanx}", "-c", "on:start,on:end", self.datafile],
            capture_output=True, text=True, check=False)
        nbin = self.nbin
        nchan_total = self.nchan
        omask = np.zeros((nchan_total, nbin), dtype=bool)      # full-size mask (all False by default)
        pattern = re.compile(r"chan=(\d+).*?on:start=([0-9,]+).*?on:end=([0-9,]+)")
        parsed = {}  # chan_id -> boolean mask for that channel
        for raw_line in psrstat_out.stdout.splitlines():
            line = raw_line.strip()
            if not line:
                continue
            ...
            try:
                on_starts = list(map(int, m.group(2).split(",")))
                on_ends   = list(map(int, m.group(3).split(",")))
                on_starts = np.array(on_starts, dtype=int)
                on_ends   = np.array(on_ends, dtype=int)
            except ValueError:
                continue
            if len(on_starts) != len(on_ends):
                continue
            mask = np.zeros(nbin, dtype=bool)
            for s, e in zip(on_starts, on_ends):
                s = max(0, min(s, nbin - 1))
                e = max(0, min(e, nbin - 1))
                if e >= s:
                    mask[s:e+1] = True
            if phase_rot!=0:
                shift   = int(round(phase_rot * self.nbin))    
                mask_final = np.roll(mask, shift)    
            else:
                mask_final = mask
            omask[chan_id, :] = mask_final
            parsed[chan_id] = mask_final
        
        #Check if some channels are empty, run Andrew's script
        #Psrchive fails to find proper mask if pulse is wrapped in phase, low SNR and wide pulses.
        selected_chans = set(chan_list)
        for chan, mask_final in enumerate(omask):
            if chan in selected_chans and np.all(mask_final==False):
                if not quiet:
                    print(f"Mask for channel {chan} empty. If profile is wrapped at the edges, try rotating first setting phase_rot.")
                    print("Otherwise, I will try another way...")
                nbin  = self.nbin
                nchan = self.nchan
                profI = self.port[0,0,chan]
                omask_chan = np.zeros((nbin), dtype=bool)
                sigmaI = 0
                sigma, onpulse_inds, _, _, _ = sigma_clip(profI,chan_num=chan,sigma_th=2,quiet=quiet)
                #Keep only indx above default sigma_on (defined at the top of this script)
                strong_inds = [idx for idx in onpulse_inds if profI[idx] > sigma_on * sigma]
                omask_chan[strong_inds] = True
                sigmaI = sigma
                omask[chan] = omask_chan

        #If all channels failed
        if np.all(omask == False):
            if not quiet:
                print("PSRCHIVE mask failed for all channels. Trying another way...")
            nbin  = self.nbin
            nchan = self.nchan
            portI = self.port[0,0]
            omask = np.zeros((nchan, nbin), dtype=bool)
            sigmaI = np.zeros(nchan)
            for i, prof2mask in enumerate(portI):
                if np.any(prof2mask==0):
                    warn=False
                sigma, onpulse_inds, _, _, _ = sigma_clip(prof2mask,chan_num=i,sigma_th=2,quiet=quiet)
                #Keep only indx above default sigma_on (defined at the top of this script)
                strong_inds = [idx for idx in onpulse_inds if prof2mask[idx] > sigma_on * sigma]
                omask[i, strong_inds] = True
                sigmaI[i] = sigma
                #omask[i, onpulse_inds] = True
            self.onpulse_mask_f = omask
            self.sigmaI = sigmaI
            failed_chans = np.where(~self.onpulse_mask_f.any(axis=1))[0]
            if len(failed_chans)>0 and warn and not quiet:
                print("WARNING: on-pulse mask failed for these channels:")
                print(failed_chans)

        self.onpulse_mask_f = omask
        return omask

    def get_onpulse_mask(self,phase_rot):
        '''
        Get onpulse mask with psrchive minimum algorithm from the mean profile.

        Input:
        phase_rot: rotate phase to get proper on/off pulse region when profile is wrapped on edges
                   note: final mask will match non-rotated profile
        '''

        #Create a mean profile of total intensity to find on pulse mask
        psrstat_out = subprocess.run(["psrstat", "-j", f"r {phase_rot}", "-jF", "-c", "on:start,on:end", self.datafile],capture_output=True,text=True)
        #psrstat_out = subprocess.run(["psrstat", "-l", "chan=10-10"", -c", "on:start,on:end", mean_I],capture_output=True, text=True) #per chan
        psrstat_output = psrstat_out.stdout
        start_match    = re.search(r"on:start=([\d,]+)", psrstat_output)
        end_match      = re.search(r"on:end=([\d,]+)", psrstat_output)
        on_starts      = list(map(int, start_match.group(1).split(',')))
        on_ends        = list(map(int, end_match.group(1).split(',')))

        onpulse_mask = np.zeros(self.data.nbin, dtype=bool)
        for start, end in zip(on_starts, on_ends):
            if start <= end:
                onpulse_mask[start:end+1] = True
        #If profile was rotated to avoid wrapping on the edges, shift the mask back to match non-rotated phase bins
        if phase_rot!=0:
            shift   = int(round(phase_rot * self.nbin))    
            om_final = np.roll(onpulse_mask, shift)    
        else:
            om_final = onpulse_mask

        self.onpulse_mask = om_final
    ...
    def normalize_portrait(self, method="rms", state='Intensity'):
        """
        Normalize each channel's profile using normalize_portrait(...).
        state is either "Intensity" (I) or "Stokes" (I,Q,U,V) or "Stokes_L" (I, L, V).
        ...
        if method not in ("mean", "max", "prof", "rms", "abs"):
            ...
            if method == "prof":
                weights = self.weights[0]                     #np.shape(nchan)
                weightsx = self.weights[self.weights > 0]     #np.shape(nchanx)
            else:
                weights = weightsx = None
                
            if state=='Intensity':
                print('Normalizing total intensity portrait...')

                # Full portrait
                self.unnorm_noise_stds = np.copy(self.noise_stds) #only needed to unnormalize the portrait, np.shape(nsub, npol, nchan)
                #port: np.shape(nchan,nbin), norm_vals: np.shape(nchan)
                self.port, self.norm_values = normalize_portrait(self.port, method,state=state,
                                                                weights=weights, return_norms=True)
                self.noise_stds[0, 0] = get_noise(self.port, chans=True)  #np.shape(nsub, npol, nchan)
                self.flux_prof = self.port.mean(axis=1)

                # Condensed portrait
                self.unnorm_noise_stdsxs = np.copy(self.noise_stdsxs) #only needed to unnormalize the portrait
                self.portx = normalize_portrait(self.portx, method,state=state,
                                                weights=weightsx, return_norms=False)
                self.noise_stdsxs = get_noise(self.portx, chans=True)  #np.shape(nchanx)
                self.flux_profx   = self.portx.mean(axis=1)               

            if state=="Stokes" or state=="Stokes_L":
                print("Normalizing Stokes portraits...")
                # Full portrait
                self.unnorm_noise_stds = np.copy(self.noise_stds) #np.shape(nsub, npol, nchan), #only needed to unnormalize the portrait
                norm_port              = np.zeros(self.port.shape)
                norm_values            = np.zeros((self.nsub, self.npol, self.nchan))
                norm_noise_stds        = np.zeros(self.unnorm_noise_stds.shape)
                for isub, sub in enumerate(self.port):
                    for ipol, pol in enumerate(sub):
                        #pol: np.shape(nchan,nbin), norm-values_pol: np.shape(nchan)
                        norm_pol, norm_values_pol = normalize_portrait(pol, 'prof', state='Stokes', weights=weights, 
                                                                       return_norms=True)
                        noise_stds = get_noise(norm_pol, chans=True)   #output array np.shape(nchan)
                        norm_port[isub,ipol] = norm_pol
                        norm_values[isub, ipol, :] = norm_values_pol
                        norm_noise_stds[isub,ipol,:] = noise_stds
                self.port        = norm_port
                self.norm_values = norm_values
                self.noise_stds  = norm_noise_stds
                self.flux_prof   = self.port.mean(axis=3) #averages over phase bins, resulting in np.shape (1,4, 416)

                # Condensed (I,Q,U,V) portrait, this can be done in one loop above (sub, pol numbers are the same, only number of channels changes)
                self.unnorm_noise_stdsxs = np.copy(self.noise_stdsxs) #np.shape(nsub,npol,nchan)
                norm_portx               = np.zeros(self.portx.shape)
                norm_noise_stdx          = np.zeros(self.unnorm_noise_stdsxs.shape)
                for isub, sub in enumerate(self.portx):
                    for ipol, pol in enumerate(sub):
                        norm_polx = normalize_portrait(pol, 'prof', state='Stokes', weights=weightsx, 
                                                       return_norms=False)
                        noise_stdsx = get_noise(norm_polx, chans=True)  #output array np.shape(nchan)
                        norm_portx[isub,ipol] = norm_polx
                        norm_noise_stdx[isub,ipol,:] = noise_stdsx
                self.portx          = norm_portx
                self.noise_stdsxs   = norm_noise_stdx
                self.flux_profx     = self.portx.mean(axis=3) #averages over phase bins, resulting in np.shape (1,4, 416)

                if state=="Stokes_L":
                    #print("Normalizing Stokes portraits (I, Q, U, V)")
                    ## Get L data (I, V normalized above) and save normalized portL (normI, normL, normV)
                    self.Lunnorm_noise_stds  = np.copy(self.Lnoise_stds)               #np.shape(nsub,3,nchan)
                    Lnorm_port               = np.zeros(self.portL.shape)              #np.shape(nsub,3,nchan,nbin)
                    Lnorm_values             = np.zeros((self.nsub, 3, self.nchan))
                    Lnorm_noise_stds         = np.zeros(self.Lunnorm_noise_stds.shape)
                    for isub in range(self.nsub):
                        normI = self.port[isub,0]           #normalized I
                        normV = self.port[isub,3]           #normalized V
                        normIval = self.norm_values[isub,0] #np.shape(nchan)
                        normVval = self.norm_values[isub,3] #np.shape(nchan)
                        normIns  = self.noise_stds[isub,0]  #np.shape(nchanx)
                        normVns  = self.noise_stds[isub,3]
                    
                        #some will be nans...
                        normL, normLval = normalize_portrait(self.portL[isub,1], 'prof', state='Stokes', weights=weights, return_norms=True)
                        normLns         = get_noise(normL, chans=True)       #output array np.shape(nchan)
                    
                        Lnorm_port[isub]       = np.stack((normI, normL, normV))
                        Lnorm_values[isub]     = np.stack((normIval, normLval, normVval))
                        Lnorm_noise_stds[isub,:] = np.stack((normIns, normLns, normVns))

                    self.portL        = Lnorm_port
                    self.Lnorm_values = Lnorm_values
                    self.Lnoise_stds  = Lnorm_noise_stds
                    #TODO this should be weighted mean 
                    self.Lflux_prof   = self.portL.mean(axis=2) #averages over phase bins, resulting in np.shape (1,4, 416)
                    
                    # Condensed (I,L,V) portrait
                    self.Lunnorm_noise_stdsxs  = np.copy(self.Lnoise_stdsxs) #np.shape(nsub,3,nchan)
                    Lnorm_portx                = np.zeros(self.portLx.shape)
                    Lnorm_noise_stdsx          = np.zeros(self.Lunnorm_noise_stdsxs.shape)
                    for isub in range(self.nsub):
                        normIx    = self.portx[isub,0]           #normalized I
                        normVx    = self.portx[isub,3]           #normalized V
                        normInsx  = self.noise_stdsxs[isub,0]
                        normVnsx  = self.noise_stdsxs[isub,3]
                    
                        normLx  = normalize_portrait(self.portLx[isub,1], 'prof', state='Stokes', weights=weightsx, 
                                                                   return_norms=False)
                        normLnsx               = get_noise(normLx, chans=True)  #output array np.shape(nchan)
                        Lnorm_portx[isub]      = np.stack((normIx, normLx,normVx))
                        Lnorm_noise_stdsx[isub,:] = np.stack((normInsx, normLnsx, normVnsx))
                    
                    self.portLx           = Lnorm_portx
                    self.Lnoise_stdsxs    = Lnorm_noise_stdsx
                    self.Lflux_profx      = self.portLx.mean(axis=2)

        print("Portrait(s) normalized!")

    def unnormalize_portrait(self):
        """
        Undo normalize_portrait.
        Works only with state='Intensity'. 
        """
    ...
    def fit_flux_profile(self, state="Intensity", channel_errs=None, nu_ref=None, guessA=1.0,
        ...
        #if channel_errs is None: channel_errs = np.ones(len(self.freqsxs[0])) #default
        if channel_errs is None:
            channel_errs = self.noise_stdsxs[0, 0] if state == "Stokes" else self.noise_stdsxs
        flux_prof = self.flux_profx[0, 0] if state == 'Stokes' else self.flux_profx
        fp = fit_powlaw(flux_prof, np.array([guessA, guessalpha]),
        ...
        if plot or savefig:
            ...
            ax2.plot(plot_freqs, powlaw(plot_freqs, fp.nu_ref, fp.amp,
                                        fp.alpha), 'k-')
            ax2.errorbar(self.freqsxs[0], flux_prof, channel_errs,
                        fmt='r+')
    ...
    def rotate_stuff(self, phase=0.0, DM=0.0, ichans=None, ichanxs=None,
        ...
        print("check")
        print(self.freqs[0].shape)
        print(self.port.shape)
        print(ichans.shape)
    ...
    def write_model_archive(self, outfile=None, state='Intensity', resultdir='.', quiet=False):
        """
        ...
        #weights = self.weights #default, but probably should be ones as below
        weights = np.ones((1,len(self.freqs[0])))
        ...
        if state=="Intensity":
            if hasattr(self, 'model') and hasattr(self, 'arch'):
                ...
                if not outfile:
                    outfile=f'{resultdir}/{self.source}_I.templ'
                for isub in range(nsub):
                    for ipol in range(npol):
                        model_data[isub, ipol] = self.model #NOTE: this is model at portrait frequencies, not arbirary frequencies
                unload_new_archive(model_data, self.arch, outfile, DM=0.0, dmc=0,
                                   weights=weights, quiet=quiet)         
        else:
            if hasattr(self, 'model') and hasattr(self, 'arch'):
                npol = 3 if state=='Stokes_L' else self.npol
                #outfile = resultdir + '/' + self.source + '_wb.%sp.templ0'%npol
                outfile = f'{resultdir}/{self.source}_wb.{npol}.templ0'
                #use portrait to set up template bands and frequencies
                #NOTE: this is model at portrait frequencies, not arbitrary frequencies
                arch       = self.arch      
                model_data = self.model   

                unload_new_archive(model_data, arch, outfile, state=state, DM=0.0, dmc=0,
                                            weights=weights, quiet=quiet)


    def write_model_archive2(self, dummy_arch=None, state='Intensity', resultdir=None, quiet=False):
        """
        Write a PSRCHIVE archive to outfile containing the model.

        outfile is the name of the written archive (source_wb.templ)
        quiet=True suppresses output.

        Only works if input datafile is a PSRCHIVE archive. The written
            archive will have the same weighted channels as the input datafile.

        If resultdir is None, outfile will be saved in current directory.
        1p, 4p in outfile denotes number of polarisations (either total intensity or Stokes)
        """
        if hasattr(self, 'model') and hasattr(self, 'arch'):
            npol = 3 if state=='Stokes_L' else self.npol
            ...
            if state=='Stokes' or state=='Stokes_L': #arch.get_data().shape will always return a full array (nsub,npol,nchan,nbin)
                                #Stokes model also is a full array
                model_data = self.model 

            else: 
                model_data = np.zeros((self.nsub, npol, self.nchan, self.nbin))
                for isub in range(self.nsub):
                    for ipol in range(npol):
                            model_data[isub, ipol] = self.model
            if dummy_arch is None:
                arch = self.arch
                outfile = self.source + '_wb.%sp.templ'%npol if resultdir is None else resultdir + '/' + self.source + '_wb.%sp.templ'%npol
            else:
                arch = pr.Archive_load(dummy_arch)
                sfx  = arch.get_filename()[-4:]   #TODO hardcoded, fixt his
                outfile = self.source + '_wb.%sp.templ.%s'%(npol,sfx) if resultdir is None else resultdir + '/' + self.source + '_wb.%sp.templ.%s'%(npol,sfx)
            ...
            if state=="Intensity":
                ...
                unload_new_archive(model_data, arch, outfile, state="Stokes", DM=0.0, dmc=0,
                                    weights=weights, quiet=quiet)

    
    def phase_resolved_alpha(self, plot=False, err_th=0.5, phase_rot=0.0, savefig=False, resultdir='.'):
        '''
        Calculate phase-resolved spectral indices based on the average data portrait. 

        Output:

        self.alphas_ph     : phase-resolved spectral indices (np.shape(nchan,nbin))
        self.alphas_ph_err : ... and their uncertainties (np.shape(nchan,nbin))
        plot               : optional
        '''
        freqs = self.freqs[0]
        nbin  = self.nbin
        I     = self.port[0,0]
        Ie    = np.tile(self.noise_stds[0,0][:, np.newaxis], (1, nbin))
        
        al_bins        = np.full(nbin, np.nan, dtype=float) #alphas per bin
        al_err_bins    = np.full(nbin, np.nan, dtype=float)

        for ibin in range(nbin):
            S_bin    = I[:, ibin].astype(float)
            Serr_bin = Ie[:, ibin].astype(float)

            # mask: only finite, positive flux & error
            m = np.isfinite(S_bin) & np.isfinite(Serr_bin) & (S_bin > 0) & (Serr_bin > 0)
            nu = freqs[m]
            S  = S_bin[m]
            Se = Serr_bin[m]
            ...
            #Number of points
            npts = nu.size
            #Calculate only if there are at least 7 frequency channels
            if npts < 7:
                continue
            try:
                popt, pcov = curve_fit(powerlaw, nu, S, sigma=Se, absolute_sigma=True, maxfev=20000)
                s0, al = popt
                s0_err, al_err = np.sqrt(np.diag(pcov))
                al_bins[ibin]     = al
                al_err_bins[ibin] = al_err
            except Exception:
                pass

        self.alphas_ph     = al_bins
        self.alphas_ph_err = al_err_bins

        if plot:
            phase = np.linspace(0, 1, nbin, endpoint=False)
            mask_bins = (al_err_bins < err_th) & np.isfinite(al_bins)
            phase_shifted = (phase[mask_bins] - phase_rot) % 1.0
            plt.errorbar(phase_shifted, al_bins[mask_bins], yerr=al_err_bins[mask_bins],fmt='o', color='black', ecolor='green', alpha=0.5, zorder=2, ms=2)
            plt.xlabel("Phase")
            plt.ylabel("Spectral index")
        
            if savefig:
                if not outname:
                    outname = self.source + f'_alpha_resolved'
                plt.savefig(f'{resultdir}/{outname}.png', dpi=300)
                print(f"Figure saved as {resultdir}/{outname}.png")


    def show_data_portrait(self, state='Intensity', resultdir='', savefig=False, outname=None, yticks_num=None, figs=(15,15),**kwargs):
        """
        Show the data portrait.
        state = 'Stokes' to plot I,Q,U,V, "Stokes_L" to plot (I,L,V), 'Intensity" to show just total intensity (default)
        yticks_num: number of yticks on flux plot. Helps with identyfing outliers. 
        ...
        if state=='Intensity':
            show_portrait(self.port * self.masks[0, 0], self.phases, self.freqs[0],
                            title, True, True, bool(self.bw < 0),resultdir=resultdir, 
                            savefig=savefig,outname=outname, **kwargs)
            ## TypeError: check if state corresponds to self.npol
        else:
            if state=='Stokes':
                show_portrait(self.port * self.masks, self.phases, self.freqs,
                            title, True, True, bool(self.bw < 0), state=state, resultdir=resultdir, 
                            savefig=savefig,outname=outname,yticks_num=None, figs=figs, **kwargs)
            if state=='Stokes_L':
                show_portrait(self.portL * self.masks[:, :3, :, :], self.phases, self.freqs,
                            title, True, True, bool(self.bw < 0), state=state, resultdir=resultdir, 
                            savefig=savefig, outname=outname, yticks_num=None, figs=figs, **kwargs)
    ...
    def show_model_fit(self, state='Intensity', resultdir='', savefig=False, **kwargs):
        """
        ...
        if state=='Intensity':
            ...
            titles = ("%s_DataPortrait"%self.source, "%s" % self.model_name[:-4], "Residuals")
            show_residual_plot(self.port, self.model, resids, self.phases,
                            self.freqs[0], self.noise_stds[0, 0], 0, titles,
                            bool(self.bw < 0), resultdir=resultdir, savefig=savefig,
                            **kwargs)
        ...
        if state=='Stokes':
            ...
            for isub in range(nsub):
                for ipol in range(npol):
                    resids = self.port[isub,ipol] - self.model_masked[isub,ipol]
                    titles = ("%s_DataPortrait"%self.source, "%s"%self.model_name[isub][ipol][:-4], "Residuals")
                    show_residual_plot(self.port[isub,ipol], self.model[isub,ipol], resids, self.phases,
                                    self.freqs[0], self.noise_stds[isub,ipol], 0, titles,
                                    bool(self.bw < 0), resultdir=resultdir, savefig=savefig,
                                    **kwargs)
        ...
        if state=="Stokes_L":
            ...
            for isub in range(nsub):
                for ipol in range(npol):
                    #self.portL should be masked!
                    resids = self.portL[isub,ipol] - self.model_masked[isub,ipol]
                    titles = ("%s_DataPortrait"%self.source, "%s"%self.model_name[isub][ipol][:-4], "Residuals")
                    
                    show_residual_plot(self.portL[isub,ipol], self.model[isub,ipol], resids, self.phases,
                                    self.freqs[0], self.Lnoise_stds[isub,ipol], 0, titles,
                                    bool(self.bw < 0), resultdir=resultdir, savefig=savefig,
                                    **kwargs)
                    
    def show_pol_profiles(self, phase_rot=0, savefig=False, resultdir='.', figsize=(6,15), aspect=None):
        '''
        For details see show_polarimetric_profiles()
        '''
        if aspect is None:
            aspect = [self.nchan, 1, self.nbin]
        else:
            aspect = aspect + [self.nbin]
        freqs  = self.freqs[0]
        source = self.source

        show_polarimetric_profiles(self.portL, source=source, freqs=freqs, phase_rot=phase_rot, savefig=savefig, resultdir=resultdir,figsize=figsize, aspect=aspect)

    def show_onpulse_mask(self, mtype='chan', savefig=False, resultdir='.',figsize=(6,15), aspect=None):
        ''' 
        For details see show_om().
        '''
        ...
        source = self.source
        if mtype=='chan':
            om_show = self.onpulse_mask_f
        else:
            om_show = self.onpulse_mask

        show_om(self.portL, mtype, freqs, source, om_show, savefig, resultdir, figsize, aspect)
        
    def show_waterfall_plot(self,phase_rot=0.0, rangeL=[0,1],rangeV=[-0.3,0.3],savefig=False,resultdir='.'):
        ''' 
        Input:
        rangeL : vmin,vmax for L/I plot, default [0,1]
        rangeV : vmin,vmax for V/I plot, default [-0.3,0.3]
        '''

        LdivI = self.Lfra_ph
        VdivI = self.Vfra_ph

        phase_shift_fraction = - phase_rot
        num_cols = LdivI.shape[1]
        shift_pixels = int(phase_shift_fraction * num_cols)

        LdivI_shifted = np.roll(LdivI, shift_pixels, axis=1)
        VdivI_shifted = np.roll(VdivI, shift_pixels, axis=1)

        phase = np.linspace(0, 1, 1024, endpoint=False)
        #TODO: freqs or freqsxs?
        freqs = self.freqsxs[0]

        extent = [phase.min(), phase.max(), freqs.min(), freqs.max()]
        ...
        #######################################
        im0 = axs[0].imshow(LdivI_shifted, aspect='auto', interpolation='none',
                            vmin=rangeL[0], vmax=rangeL[1],
                            cmap='viridis',
                            extent=extent, origin='lower')
        axs[0].set_xlabel("Phase", fontsize=15)
        axs[0].set_ylabel("Frequency [MHz]", fontsize=15)
        axs[0].set_title("L / I")

        cbar0 = fig.colorbar(im0, ax=axs[0], fraction=0.046, pad=0.04)

        #######################################
        # Net Circular Polarisation
        #######################################
        im1 = axs[1].imshow(VdivI_shifted, aspect='auto', interpolation='none',
                            vmin=rangeV[0], vmax=rangeV[1],
                            cmap='viridis',  
                            extent=extent, origin='lower')
        axs[1].set_xlabel("Phase", fontsize=15)
        axs[1].set_title("V / I")
        ...
        fig.suptitle(f"{self.source}", fontsize=16)

        plt.tight_layout(rect=[0, 0, 1, 1])
        plt.show()

        if savefig:
            outname = self.source + f'_waterfall'
            fig.savefig(f'{resultdir}/{outname}.png', dpi=300)
            print(f"Figure saved as {resultdir}/{outname}.png")

    def show_fractions(self, snr_th=1, Lerr_th=0.1, Verr_th=0.1, savefig=False, resultdir='.',):
        ''' 
        Plot phase-averaged polarisation fractions. 

        Input:
        snr_th  : plot only if SNR of L and V is above the threshold
        Lerr_th : plot only if uncertainty of fraction of L is below this threshold
        Verr_th : plot only if uncertainty of fraction of V is below this threshold
        '''

        #Barred fractions
        Lfra_b        = self.POL.Lfra_bar
        Lfra_b_err    = self.POL.Lfra_bar_err
        Vfra_b        = self.POL.Vfra_bar
        Vfra_b_err    = self.POL.Vfra_bar_err
        absVfra_b     = self.POL.absVfra_bar
        absVfra_b_err = self.POL.absVfra_bar_err

        #TODO: from xs or full?
        SNRL = self.LSNRs[0,1][self.nchanx_keep]
        SNRV = self.LSNRs[0,2][self.nchanx_keep]

        freqs = self.freqs[0][self.nchanx_keep]
        ...
        # Fractional Linear Polarisation
        #######################################
        maskL = (SNRL > snr_th) & (Lfra_b_err < Lerr_th)
        sc0 = axs[0].scatter(freqs[maskL], Lfra_b[maskL], c=SNRL[maskL], cmap='viridis', s=30)
        axs[0].errorbar(freqs[maskL], Lfra_b[maskL], yerr=np.abs(Lfra_b_err[maskL]), fmt='none', ecolor='k', alpha=0.5)
        ...
        # Net Circular Polarisation
        #######################################
        maskV = (SNRV > snr_th) & (Vfra_b_err < Verr_th)
        sc1 = axs[1].scatter(freqs[maskV], Vfra_b[maskV], c=SNRV[maskV], cmap='viridis', s=30)
        axs[1].errorbar(freqs[maskV], Vfra_b[maskV], yerr=Vfra_b_err[maskV], fmt='none', ecolor='k', alpha=0.5)
        axs[1].set_xlabel('Frequency [MHz]')
        axs[1].set_title(f'{self.source}\n\nV/I')

        #######################################
        # Absolute Circular Polarisation
        #######################################
        sc2 = axs[2].scatter(freqs[maskV], absVfra_b[maskV], c=SNRV[maskV], cmap='viridis', s=30)
        axs[2].errorbar(freqs[maskV], absVfra_b[maskV], yerr=absVfra_b_err[maskV], fmt='none', ecolor='k', alpha=0.5)
        axs[2].set_xlabel('Frequency [MHz]')
        axs[2].set_title(r"$\left|\mathrm{V}\right|/\mathrm{I}$")

        # SNR color bar
        cbar = fig.colorbar(sc0, ax=axs, orientation='vertical', fraction=0.02, pad=0.01)
        cbar.set_label("SNR")

        plt.show()

        if savefig:
            outname = self.source + f'_fractions'
            fig.savefig(f'{resultdir}/{outname}.png', dpi=300)
            print(f"Figure saved as {resultdir}/{outname}.png")
...
def get_outliers(projected_port, tck, freqs, weights=None, ncoord=None, icoord=None, stokes=0):
        """
        Finds outying residuals
        ...
        savefig if True will save fig to a given directory.
        """
        ...
        #nchan=np.shape(freqs)[0]
        #print("nchan", nchan, nprof)
        interp_freqs = freqs
        #interp_freqs= np.linspace(freqs.min(), freqs.max(), nprof)
        #print(interp_freqs)
        ...
        print(nprof)
        #print(proj_port_interp)
        #print(projected_port)
        #print(projP)
        fname="zap_Stokes" + str(stokes) + ".dat"
        f=open(fname, "w")
        f.write("\" ")
        for icoord in range(ncoord):
            rms=np.std(proj_port_interp[:,icoord] -  projected_port[:,icoord])
            print(rms)
            for i in range(nprof):
                # print(interp_freqs[i,0], freqs[i,0],proj_port_interp[i,0], projected_port[i,0])   
                if np.abs(proj_port_interp[i,icoord] -  projected_port[i,icoord]) > 3*rms:
                #    print("here")
                    print(freqs[i],proj_port_interp[i,icoord], projected_port[i,icoord], proj_port_interp[i,icoord]- projected_port[i,icoord], 3*rms)
                    if icoord < 2:
                        f.write(str(freqs[i])+ " ")
        f.write("\"")
        f.close()
...
def show_om(portL, mtype, freqs, source, om_show, savefig, resultdir, figsize, aspect):
    ''' 
    Show onpulse mask either per frequency channel or for average profile. 

    Input:
    portL: data portrait with I, L, V of shape (nsub=1,npol=3,nchan,nbin)
    mtype: "chan" for mask per channel or "average" for average profile
    freqs: centre frequencies of each nchan 
    source: source name
    om_show : on-pulse mask (per frequency channel or for average profile)
    savefig: set to True if you would like to save the figure
    resultdir: defualt is current directory
    outname: default is source_onpulse_mask_mtype.png
    figsize: default is set to work with 8 channels plot
    aspect: list of 2 [nrows = plot rows, ncol = plot cols]
    '''
    nrows,ncol,nbin = aspect[0],aspect[1],aspect[2]
    phase = np.linspace(0, 1, nbin, endpoint=False)

    if mtype=='chan':
        fig, axes = plt.subplots(nrows=nrows, ncols=ncol, sharex=True, figsize=figsize)  
        axes = np.ravel(axes)   # flatten axes into 1D list
        # zip reversed indices with axes in normal order
        for i, ax in zip(reversed(range(len(axes))), axes):
            profI = portL[0, 0, i]
            om = om_show[i]

            ax.plot(phase, profI, label='I',color='black')
            ax.scatter(phase[om], profI[om],color='goldenrod') 
            ax.set_ylabel('Flux [mJy]')
            txt = f"{freqs[i]:.2f} MHz"
            ax.text(0.98, 0.60, txt, transform=ax.transAxes,
                    ha='right', va='top', fontsize=10)
        axes[-1].set_xlabel('Phase')
        plt.show()
    else: 
        #Unweighted mean
        profI = portL[0,0].mean(axis=0)
        om = om_show

        plt.plot(phase, portL[0,0].mean(axis=0),color='black')
        plt.scatter(phase[om], portL[0,0].mean(axis=0)[om],color='goldenrod')

    if savefig:
        outname = source + f'_onpulse_mask_{mtype}'
        fig.savefig(f'{resultdir}/{outname}.png', dpi=300)
        print(f"Figure saved as {resultdir}/{outname}.png")

def show_polarimetric_profiles(portL, freqs, source, phase_rot, savefig, resultdir, figsize, aspect):
    ''' 
    Plot polarisation profiles (I, L, V) in specified channels. Default plotting options for 8 frequency channels.

    Input:
    portL: data portrait with I, L, V of shape (nsub=1,npol=3,nchan,nbin)
    freqs: centre frequencies of each nchan 
    source: source name
    phase_rot: specify if you would like to rotate the profiles in phase, default 0.0
    savefig: set to True if you would like to save the figure
    resultdir: defualt is current directory
    outname: default is source_8prof.png
    figsize: default is set to work with 8 channels plot
    aspect: list of 2 [nrows = plot rows, ncol = plot cols]
    '''
    nrows,ncol,nbin = aspect[0],aspect[1],aspect[2]
    ...
    for i, ax in zip(reversed(range(len(axes))), axes):
        ...
        phase_rot = phase_rot
        profI_rot = rotate_profile(profI, phase=phase_rot)
        profL_rot = rotate_profile(profL, phase=phase_rot)
        profV_rot = rotate_profile(profV, phase=phase_rot)

        ax.plot(phase, profI_rot, label='I', color='black')
        ax.plot(phase, profL_rot, label='L', color='goldenrod')
        ax.plot(phase, profV_rot, label='V', color='indianred')
    ...
    if savefig:
        outname = source + '_8prof'
        fig.savefig(f'{resultdir}/{outname}.png', dpi=300)
        print(f"Figure saved as {resultdir}/{outname}.png")

def sigma_clip(prof, sigma_diff_tol=1e-4, max_niter=30, width_pad=0.2, boxcar_width=64, onpulse_bool_diff_thresh=0.1, do_plot=False, chan_num=0, sigma_th=2, quiet=True):
    '''
    Find onpulse, offpulse regions and estimate offpulse noise. 
    Accepts 1D array (a profile).
    ...
    if np.all(prof == 0):
        #Don't analyse empty profile
        return 0, [], [], [], []
    else:
        prof_orig = prof.copy()
        sigma = np.nanstd(prof)
        for i in range(max_niter):
            sigma_old = sigma
            prof[np.abs(prof)>sigma_th*sigma] = np.nan
            sigma = np.nanstd(prof)
            diff = np.abs(sigma-sigma_old)
            if diff <= sigma_diff_tol:
                break

        onpulse_bool = np.isnan(prof)
        boxcar = np.zeros_like(prof)
        boxcar[boxcar.shape[0]//2-boxcar_width//2:boxcar.shape[0]//2+boxcar_width//2] = 1
        #convolve with concatenated pulse train to enforce circular boundary condition
        onpulse_bool_conv = np.convolve(boxcar, np.concatenate([onpulse_bool.astype(int)]*3), mode="same")
        #now cut out the middle to return back to standard pulse shape
        onpulse_bool_conv = onpulse_bool_conv[prof.shape[0]:2*prof.shape[0]]

        #onpulse_bool_conv /= np.max(onpulse_bool_conv) #will trigger warning if some channels are missing
        #Fix:
        m = np.max(onpulse_bool_conv)
        if m > 0:
            with np.errstate(divide="ignore", invalid="ignore"):
                onpulse_bool_conv = onpulse_bool_conv / m
            onpulse_bool = np.round(onpulse_bool_conv).astype(bool)
        else:
            onpulse_bool = np.zeros_like(prof, dtype=bool)
        onpulse_bool = np.round(onpulse_bool_conv).astype(bool)
        onpulse_bool_diff = np.diff(onpulse_bool.astype(int))
        #left_edges = np.argwhere(onpulse_bool_diff > onpulse_bool_diff_thresh).squeeze()
        #right_edges = np.argwhere(onpulse_bool_diff < -onpulse_bool_diff_thresh).squeeze()
        left_edges = np.argwhere(onpulse_bool_diff > onpulse_bool_diff_thresh).ravel() #use ravel instead of squeeze to always get 1D arrays
        right_edges = np.argwhere(onpulse_bool_diff < -onpulse_bool_diff_thresh).ravel()
        if left_edges.size!=right_edges.size:
            if not quiet:
                print(f"WARNING: when calculating onpulse mask in frequency channel {chan_num}, I found uneven edges.")
            n = min(left_edges.size, right_edges.size)
            if n == 0:
                left_edges = np.array([])
                right_edges = np.array([])
            left_edges, right_edges = left_edges[:n], right_edges[:n]
        #print("found edges",left_edges, right_edges)
        if left_edges.ndim==0:
            left_edges = np.array([left_edges])
            right_edges = np.array([right_edges])
        if np.any(left_edges > right_edges):
            right_edges = np.roll(right_edges, -1)

        final_onpulse = np.zeros_like(prof)
        final_left_edges = []
        final_right_edges = []
        for left_edge, right_edge in zip(left_edges, right_edges):#, widths):
            
            #print(left_edge, right_edge)
            if left_edge > right_edge:
                right_edge_tmp = prof.shape[0] + right_edge
                width = right_edge_tmp - left_edge
            else:
                width = right_edge - left_edge
            
            width_shift = int(round(width_pad*width))
            #print("widths",width, width_pad, max(1, width_shift))
            final_left_edge = left_edge - max(1, width_shift)
            final_right_edge = right_edge + max(1, width_shift)
            if final_right_edge > prof.shape[0]:
                final_right_edge -= prof.shape[0]
            if final_left_edge < 0:
                #final_left_edge = prof.shape[0]-final_left_edge
                final_left_edge += prof.shape[0]
            final_left_edges.append(final_left_edge)
            final_right_edges.append(final_right_edge)
            if final_left_edge > final_right_edge:
                #print("here",final_left_edge, final_right_edge)
                final_onpulse[final_left_edge:] = 1
                final_onpulse[:final_right_edge] = 1
            else:
                final_onpulse[final_left_edge:final_right_edge] = 1
        
        #Old mask used!
        final_onpulse_bool = final_onpulse.astype(bool)
        onpulse_inds = np.argwhere(onpulse_bool).squeeze()
        offpulse_inds = np.argwhere(~onpulse_bool).squeeze()
        
        #final_onpulse_bool = final_onpulse.astype(bool)
        #onpulse_inds  = np.argwhere(final_onpulse_bool).squeeze()
        #offpulse_inds = np.argwhere(~final_onpulse_bool).squeeze()

        if do_plot:
            plt.figure(figsize=(8,8))
            plt.plot(prof_orig/np.max(prof_orig))
            for i, (left, right) in enumerate(zip(final_left_edges, final_right_edges)):
                plt.axvline(left, label=f"left edge {i}", c = f"C{i}")
                plt.axvline(right, label=f"right edge {i}", c = f"C{i}", ls="--")
            plt.legend()
            plt.show()
            
        return sigma, onpulse_inds, offpulse_inds, final_left_edges, final_right_edges

def powerlaw(nu, S0, alpha, *, nu0=1400):
    return S0 * (nu / nu0) ** alpha

def broken_power_law(nu, S_b, alpha1, alpha2, x_break):
    """
    S_b     : flux at the break (normalization at x_break)
    alpha1  : slope for nu < x_break
    alpha2  : slope for nu > x_break
    x_break : break frequency
    """
    r = nu / x_break
    return np.where(nu < x_break, S_b * r**alpha1, S_b * r**alpha2)

def G_pdf(psi, P0):
    eta = (P0/np.sqrt(2.0)) * np.cos(2.0*psi)
    return (1.0/np.sqrt(np.pi)) * (1.0/np.sqrt(np.pi) + eta*np.exp(eta**2)*(1.0 + erf(eta))) * np.exp(-0.5*P0**2)

def sigma_pa_from_P0(P0):
    if P0 <= 0:
        return np.pi/2.0
    if P0 >= 10.0:
        return 0.5 / P0
    ...
    def cdf_centered(delta):
        val, _ = quad(lambda x: G_pdf(x, P0), -delta, +delta, limit=200)
        return val - target
    return brentq(cdf_centered, 0.0, np.pi/2.0, maxiter=100, xtol=1e-10)

def sigma_pa_bins(Lx, sigma_I_bin, sigma_interp):
    """
    Lx: (nchan, nbin) debiased linear polarisation per bin
    sigma_I_bin: (nchan,) per-bin rms of Stokes I
    returns: (nchan, nbin) PA 1sigma in radians
    """
    P0 = np.clip(Lx / sigma_I_bin[:, None], 0.0, None)
    # Use table for P0<=10 and analytic for P0>10
    sig = sigma_interp(np.clip(P0, 0.0, 10.0))
    hi = P0 > 10.0
    sig[hi] = 0.5 / P0[hi]
    # Undefined where Lx<=0 or den≈0; leave as big number or set to nan:
    sig = np.where(Lx > 0, sig, np.nan)
    return sig

def get_window_size(data_shape, frac=(0.004, 0.10), min_size=(4, 3)):
    """
    Returns a window size that scales with the data shape but avoids undersizing.
    
    - frac: proportion of each axis to use.
    - min_size: minimum absolute window size for each axis.
    """
    return tuple(max(min_s, int(round(f * d))) for f, d, min_s in zip(frac, data_shape, min_size))

def debias_linpol_2d(I, Q, U, window_size=(4,20), periodic=True, quiet=True):

    """
    Routine to de-bias the linear polarization.
    Follows Muller, Beck, Krause (2017)

    Works on a dynamic spectrum or other 2D data format
    Assuming dimension of (time/phase, freq)

    Needs a moment to be calculated.

    By Andrew Zic. 
    """
    
    if not quiet:
        print("Debiasing linear potrtait. It may take a moment...")
    ...
    if periodic:
        U_refl = np.vstack([U, U, U])
        Q_refl = np.vstack([Q, Q, Q])
    else:
        U_refl = np.vstack([U[::-1, :], U, U[::-1]])
        Q_refl = np.vstack([Q[::-1, :], Q, Q[::-1]])

    #now we reflect about the frequency axis as well
    U_refl = np.hstack([U_refl[:, ::-1], U_refl, U_refl[:, ::-1]])
    Q_refl = np.hstack([Q_refl[:, ::-1], Q_refl, Q_refl[:, ::-1]])

    theta_refl = np.arctan2(U_refl,Q_refl)
    x = np.cos(theta_refl)
    y = np.sin(theta_refl)
    
    phasebins = np.arange(U.shape[0], int(2*U.shape[0]), 1, dtype="int")
    channels = np.arange(U.shape[1], int(2*U.shape[1]), 1, dtype="int")
    xm = np.zeros((phasebins.shape[0],channels.shape[0]))
    ym = np.zeros((phasebins.shape[0],channels.shape[0]))
    xmm = np.zeros((phasebins.shape[0],channels.shape[0]))
    ymm = np.zeros((phasebins.shape[0],channels.shape[0]))     
    for real_phasebin, phasebin in enumerate(phasebins):
        for real_channel, channel in enumerate(channels):
            xw = (x[phasebin-window_size[0]//2:phasebin+window_size[0]//2, channel - window_size[1]//2:channel + window_size[1]//2]).copy()
            yw = (y[phasebin-window_size[0]//2:phasebin+window_size[0]//2, channel - window_size[1]//2:channel + window_size[1]//2]).copy()

            #median
            xm[real_phasebin,real_channel] = (np.median(xw))
            ym[real_phasebin, real_channel] = (np.median(yw))

            #mask out the central pixel 
            xw = xw.copy()
            yw = yw.copy()
            xw[window_size[0]//2, window_size[1]//2] = np.nan
            yw[window_size[0]//2, window_size[1]//2] = np.nan
            
            #modified median
            xmm[real_phasebin,real_channel] = np.nanmedian(xw)
            ymm[real_phasebin,real_channel] = np.nanmedian(yw)
    ...
    #Check if 
    #L_debias = np.maximum(P_debias, 0)  #low SNR points may be negative, clip them?
    if not quiet:
...
def find_significant_eigvec(eigvec, check_max=10, return_max=10,
    ...
    **kwargs get passed to smart_smooth(...).
    ...
    eigvec_sorted = np.zeros(eigvec.shape) 
    ieig = []
    neig = 0
    snrs = []  #add sorting eigenvectors by snrs
    for ivec in range(check_max):
        add_eigvec = False
        ...
        if add_eigvec:
            ieig.append(ivec)
            snrs.append(ev_snr)
            eigvec_sorted[:,ivec] = eigvec[ivec]
            neig += 1
    ...
    ieig = np.array(ieig)
    #Sort ieig by snr
    ieig_sorted = (np.argsort(snrs))[::-1]

    if return_smooth:
        #ieig_sorted returns indices of eig sorted by snr values (default: numbers between 0-9)
        #smooth_eigvec here is not sorted, as ppspline uses ieig to sort them later (smooth_eigvec[:,ieig])
        return ieig_sorted[:return_max], smooth_eigvec
    else:
        return ieig_sorted
...
def fit_gaussian_portrait(model_code, data, init_params, scattering_index,
    ...
    print("CHECKING fit_gaussian_portrait init params (pplib)")
    print(init_params[1])
    print(fit_flags[1])
    ...
    fitted_params = np.array([param.value for param in
                              list(results.params.values())])
    print("CHECKING fit_gaussian_portrait resulted params (pplib)")
...
def plot_mask(prof, onpulse_mask, snr=0.00):
    '''
    Plot profile with onpulse mask. To check if on pulse is estimated correctly. 
    Input:
    prof: 1D profile of size (nbin, )
    onpulse_mask: a list of bool statements
    snr: [optional] given snr will be labeled on the plot 
    '''

    plt.figure(figsize=(10, 4))
    plt.plot(prof, label=f'SNR: {snr:.2f}', color='black')

    # Highlight on-pulse region
    plt.fill_between(
        np.arange(len(prof)), prof, where=onpulse_mask,
        color='red', alpha=0.3, label='On-pulse region'
    )

    plt.xlabel("Phase bin")
    plt.ylabel("Amplitude")
    plt.title("Profile with On-Pulse Mask")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
def get_SNR(prof, fudge=3.25):
    ...
    PSRCHIVE and this method don't match because of a different normalization (Weq vs Non)

    Reference: Lorimer & Kramer (2005).
    """
    noise = get_noise(prof)
    nbin = len(prof)
    #dc = np.real(np.fft.rfft(prof))[0]
    dc = 0
...
def get_SNR_better(profile, onpulse_mask=None, threshold=0.10, use_pp_noise=False, 
                    power_based=False, frac=4, plot=False, quiet=True):
    ''' 
    Calculate SNR. 
    For total intensity and 4 Stokes, SNR can be calculated with PSRchive, however it needs to be calculated separately if one needs to get L. 
    Profile: 1D profile on size (nbins, )
    onpulse_mask: array of bool (False, True,...) where on-pulse is estimated. Preferred: get from psrchive, but if None (poor) approximation will be calculated.
    power-based: only for Q,U,V in order to calculate signal properly when it oscillates around 0. 
    '''
    prof = np.asarray(profile)
    if np.all(profile == 0):
        return 0.0
    if onpulse_mask is None:
        #Calculate mask and std_noise with Andrew's script
        #TODO: actually, calculate this with psrchive, to be consistent
        sigma, onpulse_inds, offpulse_inds, _, _ = sigma_clip(profile,quiet=quiet)
        onpulse_mask = np.zeros(len(profile), dtype=bool)
        onpulse_mask[onpulse_inds] = True
        noise_std = sigma
        
    if not np.any(onpulse_mask):
        return 0.0

    offpulse_mask = ~onpulse_mask
    N_on = onpulse_mask.sum()
    if N_on == 0 or offpulse_mask.sum() == 0:
        return 0.0
    if power_based:
        #Needed for Q, U, V which might have negative flux
        signal = np.sqrt(np.sum(prof[onpulse_mask] ** 2))
    else:
        signal = prof[onpulse_mask].sum()

    if use_pp_noise or onpulse_mask is not None:
        #standard PP noise estimation method in PP
        noise_std = get_noise_PS(prof[offpulse_mask], frac=frac)

    snr = signal / (noise_std * np.sqrt(N_on))

    return snr

def estimate_onpulse_mask(profile, threshold=0.10):
    """
    Estimate on-pulse region using the cumulative profile algorithm.
    Should be similar to PSRchive but is not too good yet. 

    Input:
    profile : 1D np.ndarray
            Pulse profile (baseline should already be removed).
    threshold : float
            Fraction of total flux to define pulse edges (default 0.10 = 10%).
    ...
    """
    prof = np.asarray(profile)
    prof = prof - np.median(prof)  # flatten baseline
    prof = np.clip(prof, 0, None)  # cumulative logic needs non-negative

    cum_prof = np.cumsum(prof)
    total = cum_prof[-1]

    lower_idx = np.where(cum_prof > threshold * total)[0]
    upper_idx = np.where(cum_prof < (1 - threshold) * total)[0]
    ...
    if start >= end:
        # fallback: use peak-centered mask of fixed width
        peak = np.argmax(prof)
        width = max(1, len(prof) // 32)
        start = max(0, peak - width)
        end = min(len(prof), peak + width)

    mask = np.zeros_like(prof, dtype=bool)
...
def normalize_portrait(port, method='rms', state='Intensity', weights=None, return_norms=False):
    """
    ...
    if method not in ("mean", "max", "prof", "rms", "abs"):
        ...
        if state=='Intensity':
            ...
            if method == "prof":
                ...
                if weights is None:
                    ...
                    #Aren't weights of good ichans just 1s...?
        ...
        if state=='Stokes':
            ...
            if method == "prof":
                #Why definining something new instead of taking masks? 
                #Different definition of good_ichans than above, should be the same!
                good_ichans = np.where(weights!=0.0)[0]
...
def load_data(filename, state=None, dedisperse=False, dededisperse=False,
    ...
    if state is not None:
        #print(f"State should be {state}.")
        if state != arch.get_state():
            arch_state = arch.get_state()
            arch.convert_state(state)
            if not quiet:
                print(f"Archive is not in correct state (it's {arch_state}). Will convert to {state}.")
    ...
    #This replicates weights_norm into a new, 3rd dimension
    #Weights_norm = np.shape(1, nchan), np.ones(nbin) = np.shape(nbin,)
    #So mask will be (1, 416, 1024). 
    masks = np.einsum('ij,k', weights_norm, np.ones(nbin))

    #This mask will be of shape np.shape(1, npol, nchan, nbin)
    ...
    for isub in range(nsub):
        for ipol in range(npol):
            for ichan in range(nchan):
                SNRs[isub, ipol, ichan] = arch.get_Integration(isub).get_Profile(ipol, ichan).snr()
    ...
    if state=='Stokes':
        if flux_prof:
            arch.dedisperse()
            arch.tscrunch()
            flux_prof = arch.get_data().mean(axis=3) 
        else:
            flux_prof = np.array(([],[],[]))

    # The rest is now ignoring npol...
    # Get pulse profile (I, f/t scrunched)
    arch.tscrunch()
    arch.fscrunch()
    arch.pscrunch()
    #TODO generate an average prof of each Stokes
    ...
    if not quiet:
        ...
        # subints          = %d\n\
        # unzapped subint  = %d\n\
        pol'n state        = %s\n" % (P, DM, RM, nu0, bw, nbin, nchan, nchanx, nsub,
                                      nsubx, state))
    if refresh_arch: arch.refresh() #arch was pscrunched for some info, return it to initially desired state
    ...
    data = DataBunch(arch=arch, backend=backend, backend_delay=backend_delay,
                     bw=bw, doppler_factors=doppler_factors, DM=DM, dmc=dmc, RM=RM, 
...
def unload_new_archive(data, arch, outfile, state='Intensity', DM=None, dmc=0, weights=None,
    ...
    weights is an nsub x nchan array of channel weights; if None, nothing is
        changed.
    state: "Intensity" (archive with template I), "Stokes" (archive with template I,Q,U,V) or "Stokes_L" (archive with template I, L, V)
    quiet=True suppresses output.
    """
    if dmc:
    ...
    if state=='Stokes' or state=="Stokes_L":
        ...
        arch.set_state("Stokes") #even if Stokes_L the state will be "Stokes"
...
def show_portrait(port, phases=None, freqs=None, title=None, prof=True,
                  ...
                  state='Intensity', resultdir='', outname=None, savefig=False, yticks_num=None,figs=(15,15),**kwargs):
    """
    ...
    state is polarization state, either "Intensity" (default) or "Stokes" (I,Q,U,V) or "Stokes_L" (I, L, V).
    outname: output name of the portrait to save
    **kwargs get passed to imshow.  e.g. vmin, vmax...
    ...
    if state=='Intensity':
        ...
        if savefig:
            if outname is None:
                outname='I_port'
            plt.savefig(resultdir + f'/{outname}.png', format='png')
            plt.close()
        if show:
            plt.show()

    ###########################################################################################
    # Plot all Stokes portraits
    ###########################################################################################
    if state=='Stokes':
        states = ['I', 'Q', 'U', 'V']
        if not figs:
        ...
        for pol, ax in enumerate(axs.flat):  # Flatten the 2x2 grid to iterate over individual subplots

            ax.set_title(states[pol])

            port_pol  = port[0,pol]   #np.shape(nchan,nbin), per pol 
            freqs_pol = freqs[0]      #np.shape(nchan), per pol 

            ax.set_xticks([])
            ax.set_yticks([])

            pi = 0 # average profile
            fi = 0 # flux 
            ci = 0 # colorbar 

            if freqs_pol is None:
                freqs_pol = np.arange(len(port_pol))
                ylabel = "Channel Number"
            else:
                ylabel = "Frequency [MHz]"
            ...
            if rvrsd:
                freq_pol = freqs_pol[::-1]
                port_pol = port_pol[::-1]

            if extent is None:
                extent = (phases[0], phases[-1], freqs_pol[0], freqs_pol[-1])

            if prof or fluxprof:
                weights = port_pol.mean(axis=1)
                print("")

            if prof:
                portx    = np.compress(weights, port_pol, axis=0)
                prof_pol = portx.mean(axis=0)  # NB: this is unweighted mean, for one pol
                pi = 1

            if fluxprof:
                fluxprof_pol = port_pol.mean(axis=1)
                fluxprofx = np.compress(weights, fluxprof_pol)
                freqsx = np.compress(weights, freqs_pol)
                fi = 1
            ...
            ##############################################
            nn = 2 * 3 * 5  
            grid = gs.GridSpecFromSubplotSpec(nn, nn, subplot_spec=ax.get_subplotspec())
            
            # Create three sub-grids within each subplot
            ax1 = plt.subplot(grid[(old_div(pi * nn, 6)):, (old_div(fi * nn, 6)):])
            ax2 = plt.subplot(grid[:(old_div(pi * nn, 6)), (old_div(fi * nn, 6)):old_div(((3 + fi + ci) * nn),(4 + fi + ci))])
            ax3 = plt.subplot(grid[(old_div(pi * nn, 6)):, :(old_div(fi * nn, 6)-1)])  #-1 at the end shifts the plot to the left

            # Add these subplots to the figure
            fig.add_subplot(ax1)
            fig.add_subplot(ax2)
            fig.add_subplot(ax3)
            ...
            ## MAIN PLOT
            ##################
            im = ax1.imshow(port_pol, aspect=aspect, origin=origin, extent=extent,
                            interpolation=interpolation, **kwargs)
            if colorbar: plt.colorbar(im, ax=ax1, use_gridspec=False)
            ...
            ## Average profile
            ######################
            ax2.plot(phases, prof_pol, 'k-')

            ax2.set_xticks(ax1.get_xticks())
            ax2.set_xticklabels(())
            #ax2.set_yticks([0, round(old_div(prof_pol.max(), 2), 1), round(prof_pol.max(), 1)])
            ax2.set_xlim(phases.min(), phases.max())
            rng = prof_pol.max() - prof_pol.min()
            ax2.set_ylim(prof_pol.min() - 0.03 * rng, prof_pol.max() + 0.05 * rng)
            ax2.set_ylabel("Flux Units")
            ...
            if yticks_num:
                ...
                yticks = np.linspace(ax1.get_ylim()[0], ax1.get_ylim()[1], num=yticks_num)  # Adjust 'num' as needed
    ...
    ###########################################################################################
    # Plot all Stokes portraits (I, L, V)
    ###########################################################################################
    if state=='Stokes_L':
        states = ['I', 'L', 'V']
        ...
        fig, axs = plt.subplots(2, 2, figsize=figsize)  #set figsize=(20,40) if you want to see channels better
        ...
        for pol, ax in enumerate(axs.flat):  # Flatten the 2x2 grid to iterate over individual subplots
            ...
            if pol < 3:
                ax.set_title(states[pol])
                port_pol  = port[0,pol]   #np.shape(nchan,nbin), for one pol
                freqs_pol = freqs[0]      #np.shape(nchan), for one pol 
                ...
                # Optionally, you can remove the last axis
                fig.delaxes(ax)  # This will remove the last subplot
                # Or leave it empty
                ax.axis('off')  # This will keep the axis but hide it
...
def show_residual_plot(port, model, resids=None, phases=None, freqs=None,
                       ...
                       resultdir='', savefig=False, 
                       colorbar=True, aspect="auto", interpolation="none",
    ...
    **kwargs get passed to imshow.  e.g. vmin, vmax...
    ...
    if savefig==True:
        plt.savefig(resultdir + '/%s.resids.png'%titles[1], format='png')
        print('Plot saved at %s'%(resultdir))
        plt.close()
    else:
        plt.show()
...
def show_spline_curve_projections(projected_port, tck, freqs, weights=None,
                                  ncoord=None, icoord=None, title=None, resultdir='', savefig=False):
    """
    ...
    if savefig:
        if ncoord > 1:
            fig1.savefig(resultdir + "/%s.proj.png"%title, format='png')
            fig2.savefig(resultdir +  "/.%s.freq.png"%title, format='png')
        else:
            fig2.savefig(resultdir+ "/%s.freq.png"%title, format='png')

        print('Figures saved in %s/'%(resultdir))
...
def show_eigenprofiles(eigprofs=None, smooth_eigprofs=None, mean_prof=None,
    ...
    savefig is either True or False.
    """
    ...
    if savefig:
        fig.savefig(resultdir + '/%s.eigvec.png'%title, format='png')
        print('Figure saved at %s'%(resultdir))
        plt.close('all')
