...
# Polarimetric updates by Gosia Curylo (gosia.curylo@monash.edu).
...
def psradd_archives(metafile, outfile, palign=False,quiet=True, ephem=None):
    """
...
def align_archives(metafile, initial_guess, fit_dm=True, tscrunch=False,
        pscrunch=True, dedisperse=False, SNR_cutoff=0.0, outfile=None, norm=None, rot_phase=0.0,
        place=None, niter=1, quiet=False, resultdir=None):
    """
    ...
    model_port = (model_data.masks * model_data.subints)[0, 0]  #np.shape(nchan,nbin)
    ...
    while (niter):
        ...
        #Save fitted DMs for each iteration as txt files
        if fit_dm:
            DMs         = []  
            DMerrs      = []
        doppPP_list = []
        ...
        for ifile in range(len(datafiles)):
            try:
                ...
                #If there's some unknown problem with some file, print those to check
                #print("CHECK")
                #print(datafiles[ifile])
                #print("loaded")

            ######################################################################
            ### Make sure you load the right data 
            ######################################################################
            except RuntimeError:
                if not quiet:
            ...
            ######################################################################
            #Check if the initial template and datafiles have same nchan
            ######################################################################
            try:
            ...
            ######################################################################
            #Read DM of each observation (unless it was dedispersed: dmc=1)
            ######################################################################
            doppler_factor = data['doppler_factors']
            doppPP_list.append(doppler_factor[0])

            DM_guess = data.DM * np.logical_not(data.dmc)  # = 0.0 if de-disp'd

            for isub in data.ok_isubs:
                mjd = data.epochs[isub].in_days()
                MJDs.append(mjd)

                #Get an array of freq channels present in both data and template
                if same_freqs:
                ...
                ######################################################################
                ## Find freq_ref, rotate data by DM and fit for phase
                ######################################################################
                ...
                phase_guess = fit_phase_shift(np.average(rot_port, axis=0,
                                                         weights=data.weights[isub, ichans]), model.mean(axis=0),Ns=nbin).phase
                ...
                if len(freqs) > 1:
                    ...
                    ini_params = [phase_guess, DM_guess, 0.0, 0.0, 0.0]
                    ...
                    if fit_dm:
                        DMs.append(results.DM)
                        DMerrs.append(results.DM_err)

                else:  #1-channel hack
                    results = fit_phase_shift(port[0], model[0], errs[0],Ns=nbin)
                    results.DM = data.DM
        ...
        #Save fitted DMs as a file
        if fit_dm:
            with open(f'{resultdir}/DMfit_{count}.txt', 'w') as f:
                print("MJD", "DM", "DM err", 'Doppler factor',file=f)
                for i,j,k,l in zip(MJDs, DMs, DMerrs, doppPP_list):
                    print(i,j,k,l, file=f)
...
if __name__ == "__main__":
    ...
    parser.add_option("--resultdir",
                      ...
                      help="Path of the directory to save results (currently only DM fits). Default: current directory.")
    ...
    print(f"SNR cutoff: {SNR_cutoff}")
    print(f"fit_dm: {fit_dm}")
    print(f"Pscrunch: {pscrunch}")
    print(f'Niter: {niter}')
    print("")
    ...
    if initial_guess is None and fwhm is None:  # use psradd :(
        ...
        print("Processing initial template guess...")
        vap_cmd = "vap -n -c nchan %s" % initial_guess
        psr_cmd = f"vap -c name {initial_guess} | tail -n 1 | awk '{{print $2}}'"
        nchan = int(sub.Popen(shlex.split(vap_cmd), stdout=sub.PIPE
                              ).stdout.readlines()[0].split()[-1])
        print(nchan)
        psr = sub.check_output(psr_cmd, shell=True, text=True).strip()
        if nchan == 1:  # used constant average profile
            #tmp_file = f"ppalign.%d.tmp.fits" % np.random.randint(10, 1000)  #this might not be unique for paralel runs!
            tmp_file = f"{psr}_ppalign.%d.tmp.fits" % np.random.randint(10, 1000)
    ...
    print("Starting portrait alignment...")
    align_archives(metafile, initial_guess=initial_guess, fit_dm=fit_dm,
            ...
            niter=niter, quiet=quiet, resultdir=resultdir)
